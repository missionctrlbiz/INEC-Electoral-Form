...
// Security Check: Ensure only logged-in administrators can access this page.
require_admin();
...
$party_totals_sql = "SELECT p.id, p.acronym, SUM(ps.score) as total_votes 
                     FROM political_parties p 
                     JOIN party_scores ps ON p.id = ps.party_id 
                     GROUP BY p.id, p.acronym 
                     ORDER BY total_votes DESC";
$party_totals_result = $conn->query($party_totals_sql);
while($row = $party_totals_result->fetch_assoc()) {
    $all_parties[] = $row;
}
...
switch ($current_level) {
    case 'ward':
        $sql = "SELECT pu.id, pu.name, r.status, $select_sql 
                FROM results r JOIN party_scores ps ON r.id = ps.result_id 
                JOIN polling_units pu ON r.polling_unit_id = pu.id 
                WHERE pu.ward_id = ? GROUP BY pu.id, pu.name, r.status ORDER BY pu.name ASC";
        $param_type = "i";
        $param_value = $ward_id;
        break;
    case 'lga':
        $sql = "SELECT w.id, w.name, $select_sql 
                ...
                WHERE w.lga_id = ? GROUP BY w.id, w.name ORDER BY w.name ASC";
        $param_type = "i";
        $param_value = $lga_id;
        break;
    case 'state':
        $sql = "SELECT l.id, l.name, $select_sql 
                FROM results r JOIN party_scores ps ON r.id = ps.result_id 
                JOIN polling_units pu ON r.polling_unit_id = pu.id 
                JOIN wards w ON pu.ward_id = w.id 
                JOIN lgas l ON w.lga_id = l.id 
                WHERE l.state_id = ? GROUP BY l.id, l.name ORDER BY l.name ASC";
        $param_type = "i";
        $param_value = $state_id;
        break;
    default: // national
                ...
                <?php foreach ($breadcrumbs as $index => $crumb): ?>
                    <li class="flex items-center">
                        <?php if (!empty($crumb['link'])): ?>
                            <a href="<?php echo $crumb['link']; ?>" class="text-inec-green hover:underline"><?php echo htmlspecialchars($crumb['name']); ?></a>
                        <?php else: ?>
                            <span class="text-slate-500"><?php echo htmlspecialchars($crumb['name']); ?></span>
                        <?php endif; ?>
    ...
    <div class="bg-white rounded-xl shadow-lg p-6">
        <div class="flex flex-col md:flex-row justify-between items-center mb-4 gap-4">
            <div class="relative w-full md:w-1/3">
                ...
                <input type="text" id="filter-input" class="w-full pl-10 pr-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-inec-green" placeholder="Filter results...">
            </div>
            <?php if (count($other_parties) > 0): ?>
            <button id="toggle-parties-btn" class="bg-slate-100 text-slate-700 font-semibold px-4 py-2 rounded-lg hover:bg-slate-200 transition text-sm">
        ...
        <div class="overflow-x-auto">
            <table id="results-table" class="w-full text-sm text-left text-slate-600">
                <thead class="text-xs text-slate-700 uppercase bg-slate-100">
                    <tr>
                        <th class="px-6 py-3 cursor-pointer sortable" data-sort-dir="asc">
                            <?php 
                                if ($current_level === 'national') echo 'State';
                                elseif ($current_level === 'state') echo 'LGA';
                                elseif ($current_level === 'lga') echo 'Ward';
                                else echo 'Polling Unit';
                            ?> <i class="bi bi-arrow-down-up ml-1"></i>
                        </th>
                        ...
                        <?php foreach ($top_parties as $party): ?>
                            <th class="px-6 py-3 text-center cursor-pointer sortable" data-sort-dir="asc"><?php echo htmlspecialchars($party['acronym']); ?> <i class="bi bi-arrow-down-up ml-1"></i></th>
                        <?php endforeach; ?>
                        
                        <?php foreach ($other_parties as $party): ?>
                            <th class="px-6 py-3 text-center cursor-pointer sortable hidden toggleable-party-col" data-sort-dir="asc"><?php echo htmlspecialchars($party['acronym']); ?> <i class="bi bi-arrow-down-up ml-1"></i></th>
                        <?php endforeach; ?>
                ...
                <tbody id="results-tbody">
                    <?php if (empty($data_rows)): ?>
                        <tr><td colspan="100%" class="text-center py-10 text-slate-500">No results found for this level.</td></tr>
                    <?php endif; ?>
                    ...
                    <tr class="bg-white border-b hover:bg-slate-50">
                        <td class="px-6 py-4 font-bold text-slate-800">
                            ...
                            if ($current_level === 'national') {
                                $next_link = "national-summary.php?state_id={$row['id']}";
                                $data_level = 'state';
                            } elseif ($current_level === 'state') {
                                $next_link = "national-summary.php?lga_id={$row['id']}";
                                $data_level = 'lga';
                            } elseif ($current_level === 'lga') {
                                $next_link = "national-summary.php?ward_id={$row['id']}";
                                $data_level = 'ward';
                            } else {
                                $data_level = 'pu'; // Polling Unit, no further drill down
                            }
                            ?>
                            <a href="<?php echo $next_link; ?>" class="summary-link text-inec-green hover:underline" data-id="<?php echo $row['id']; ?>" data-level="<?php echo $data_level; ?>" data-name="<?php echo htmlspecialchars($row['name']); ?>">
                        ...
                        <?php foreach ($top_parties as $party): ?>
                            <td class="px-6 py-4 text-center"><?php echo number_format($row['party_' . $party['id']]); ?></td>
                        <?php endforeach; ?>
                        
                        <?php foreach ($other_parties as $party): ?>
                            <td class="px-6 py-4 text-center hidden toggleable-party-col"><?php echo number_format($row['party_' . $party['id']]); ?></td>
                        <?php endforeach; ?>
                        ...
                        <?php if ($current_level === 'ward'): ?>
                            <td class="px-6 py-4 text-center">
                                ...
                                ?>
                                <span class="px-2 py-1 font-semibold leading-tight text-xs rounded-full <?php echo $badge_class; ?>"><?php echo ucfirst($status); ?></span>
                            </td>
                        <?php endif; ?>
                    </tr>
                    <?php endforeach; ?>
...
<div id="summary-modal" class="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 hidden transition-opacity duration-300 opacity-0">
    <div id="modal-content" class="bg-slate-50 rounded-xl shadow-2xl w-full max-w-2xl transform transition-all duration-300 scale-95 opacity-0 max-h-[90vh] flex flex-col">
        <header class="p-4 border-b bg-white rounded-t-xl flex justify-between items-center">
            <h3 id="modal-title" class="font-display font-bold text-xl text-slate-800">Detailed Summary</h3>
            <button id="modal-close-btn" class="text-slate-500 hover:text-slate-800 text-2xl">&times;</button>
        </header>
        <div id="modal-body" class="p-6 overflow-y-auto">
            <!-- Content will be loaded via JavaScript -->
            <div class="text-center py-10">
                <p class="text-slate-500">Loading details...</p>
            </div>
        </div>
    </div>
</div>
